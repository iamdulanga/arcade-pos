<div>
    ...
    <div class="flex items-center justify-between mb-5">
        <div class="flex items-center gap-3 flex-wrap">
            ...
            {{-- Search --}}
            <input type="text" wire:model.live.debounce.200ms="search" placeholder="Search name, SKU, barcode…"
            ...
            <select wire:model.live="filterCategory"
                class="w-48 pr-8 px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400">
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-gray-100 bg-gray-50">
                    <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Product
                    </th>
                    <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">SKU</th>
                    <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Category
                    </th>
                    <th class="text-right px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Price
                    </th>
                    <th class="text-right px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Stock
                    </th>
                    <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Flags
                    </th>
            ...
            <tbody class="divide-y divide-gray-50">
                @forelse($products as $product)
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-4 py-3 text-right font-medium text-gray-800">Rs.
                            {{ number_format($product->selling_price, 2) }}</td>
                        <td class="px-4 py-3 text-right">
                            <span
                                class="font-medium {{ $product->stock_quantity === 0 ? 'text-red-500' : ($product->isLowStock() ? 'text-amber-500' : 'text-gray-800') }}">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-1.5">
                                @if($product->has_barcode)
                                    <span
                                        class="px-1.5 py-0.5 bg-blue-50 text-blue-600 text-xs rounded font-medium">Barcode</span>
                                @endif
                                @if($product->is_pinned)
                                    <span
                                        class="px-1.5 py-0.5 bg-amber-50 text-amber-600 text-xs rounded font-medium">Pinned</span>
                                @endif
                                @if(!$product->is_active)
                                    <span
                                        class="px-1.5 py-0.5 bg-gray-100 text-gray-400 text-xs rounded font-medium">Inactive</span>
                                @endif
                            </div>
                        </td>
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <button wire:click="delete({{ $product->id }})" wire:confirm="Delete {{ $product->name }}?"
            ...
            <div class="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-screen overflow-y-auto p-6">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="col-span-2">
                        <label class="block text-xs font-medium text-gray-600 mb-1">Product name <span
                    ...
                    {{-- Category --}}
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Category <span
                                class="text-red-400">*</span></label>
                    ...
                    {{-- Supplier --}}
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Supplier <span
                    ...
                    {{-- Selling price --}}
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Selling price (Rs.) <span
                                class="text-red-400">*</span></label>
                    ...
                    {{-- Cost price --}}
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Cost price (Rs.) <span
                                class="text-gray-400 font-normal">(optional)</span></label>
                    ...
                    {{-- Stock quantity --}}
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Stock quantity <span
                                class="text-red-400">*</span></label>
                    ...
                    <div class="col-span-2 border border-gray-100 rounded-xl p-4 bg-gray-50">
                        <div class="flex items-center gap-2 mb-3">
                            ...
                            <label for="has_barcode" class="text-sm font-medium text-gray-700">This item has a
                                barcode</label>
                        </div>
                        @if($has_barcode)
                            <div>
                        ...
                        @else
                            <p class="text-xs text-gray-400">A SKU will be auto-generated from the category prefix when saved.
                            </p>
                        @endif
                    </div>
                    ...
                    <div class="col-span-2 border border-gray-100 rounded-xl p-4 bg-gray-50">
                        <div class="flex items-center gap-2 mb-2">
                            <input type="checkbox" wire:model.live="is_pinned" id="is_pinned"
                                class="rounded border-gray-300 text-indigo-600" />
                            <label for="is_pinned" class="text-sm font-medium text-gray-700">Pin to quick-select grid at
                                POS</label>
                        </div>
                        @if($is_pinned)
                            <div>
                                <label class="block text-xs font-medium text-gray-600 mb-1">Grid position <span
                                        class="text-gray-400 font-normal">(optional — lower number = first)</span></label>
                    ...
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Author <span
                                class="text-gray-400 font-normal">(books only)</span></label>
                        <input type="text" wire:model="author" placeholder="e.g. S. Wijesuriya"
                            class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400" />
                    </div>

                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">ISBN <span
                                class="text-gray-400 font-normal">(books only)</span></label>
                    ...
                    <div class="col-span-2 flex items-center gap-2">
                        <input type="checkbox" wire:model="is_active" id="prod_is_active"
                            class="rounded border-gray-300 text-indigo-600" />
                        <label for="prod_is_active" class="text-sm text-gray-600">Active — visible at POS and in
                            reports</label>
                    </div>
